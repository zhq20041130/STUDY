...
\emph{例题6:}\\
抗磁性和朗道能级的推导。\\
现考虑在磁场$B=B_0 \cdot \vec{e_z}$中运动的电子：
\[
    \hat{H} = \frac{\hat{\pi}}{2m} = \frac{(\hat{\vec{P}}-q\vec{A})^2}{2m} = \frac{(\hat{\vec{P}}+e\vec{A})^2}{2m}
\]\\
...
则：
\[
    \hat{H}=\frac{1}{2m}[(\hat{P}_x-eBy)^2+\hat{p}_y^2+\hat{p}_z^2]
\]

注意到在此哈密顿量中只存在"y",没有x，z，则$[\hat{H},\hat{P}_x]=[\hat{H},\hat{P}_x]=0$
因此我们可以取CSCO为$(\hat{H},\hat{P}_x,\hat{P}_z)$,设共同本征函数为：
\[
    \psi(x,y,z) = e^{\frac{i p_x x}{\hbar}} e^{\frac{i p_z z}{\hbar}}\cdot \phi(y)
\]
将哈密顿量和试探解带入定态薛定谔方程$\hat{H}\psi = E \psi$:
\[
    \frac{1}{2m}[(\hat{P}_x-eBy)^2+\hat{p}_y^2+\hat{p}_z^2]e^{\frac{i p_x x}{\hbar}} e^{\frac{i p_z z}{\hbar}}\cdot \phi(y) = E e^{\frac{i p_x x}{\hbar}} e^{\frac{i p_z z}{\hbar}}\cdot \phi(y)
    \rightarrow
\]
\[
    \frac{1}{2m}[(P_x-eBy)^2+\hat{p}_y^2]\phi(y) = (E-p_z^2)\phi(y)
\]

注意到上面的方程与一维问题中y方向的谐振子方程很相似:
\[
    [\frac{\hat{P}_y^2}{2m}+\frac{1}{2}m\omega^2y^2]\psi = E\psi
\]
我们可以令：
\[
    \xi = y-\frac{p_x}{eB},\omega^2 = \frac{e^2B^2}{m^2},E' = E - \frac{p_z^2}{2m}
\]
原方程可化为：
\[
    [\frac{\xi^2}{2m}+\frac{1}{2}m\omega^2\xi^2]\phi(\xi) = E'\phi(\xi)
\]
其解为：
\[
...
\[
    \phi(\xi) = N_n H_n(\alpha \xi)e^{-\frac{\alpha^2 \xi^2}{2}}
\]
其中$\alpha = \sqrt{\frac{eB}{\hbar}}$

换回原变量：
\[
    E_n = (n+\frac{1}{2})\hbar \omega + \frac{p_z^2}{2m}
\]
\[
    \phi(y) = N_n H_n[\alpha (y-\frac{p_x}{eB})]e^{-\frac{\alpha^2 (y-\frac{p_x}{eB})^2}{2}}
\]
这里的最终能量为一个一维谐振子能量加上Z方向的自由粒子能量，我们称为$\emph{朗道能级}$\\
在经典情况下，$U = -\vec{M} \cot \vec{B}$
\[
    (n+\frac{1}{2})\hbar \omega + \frac{p_z^2}{2m} = -M_z B \rightarrow
\]
\[
    M_z = -\frac{1}{B} [(n+\frac{1}{2})\hbar \omega + \frac{p_z^2}{2m}]
\]
由于这里的负号，与外磁场方向相反，这就是抗磁性的来源
\end{document}